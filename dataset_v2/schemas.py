...
def trial_schema() -> dict:
    return {
        ...
        "description": (
            "One trials/trial_manifest.csv row (Phase 7): an easy/medium/hard initial-state trial "
            "for one trajectory. q_initial is drawn independently from the operational joint limits "
            "and classified only against the trajectory's first canonical target pose; q_reference "
            "is never used and never appears here (it stays in the protected evidence tree)."
        ),
        "type": "object",
        "required": [
            "trial_id",
        ...
        "properties": {
            ...
            "trajectory_family": {"type": "string", "enum": ["core", "random_challenge"]},
            "split": {"type": "string", "enum": SPLIT_ENUM},
            "difficulty": {"type": "string", "enum": INIT_CLASS_ENUM},
            "q_initial": {"type": "array", "items": {"type": "number"}, "minItems": 7, "maxItems": 7},
            "initial_position": {"type": "array", "items": {"type": "number"}, "minItems": 3, "maxItems": 3},
            "initial_quaternion_wxyz": _QUATERNION_WXYZ,
            "first_target_position": {"type": "array", "items": {"type": "number"}, "minItems": 3, "maxItems": 3},
            "first_target_quaternion_wxyz": _QUATERNION_WXYZ,
            "initial_position_error_m": {"type": "number", "minimum": 0},
            "initial_orientation_error_rad": {"type": "number", "minimum": 0},
            "primary_difficulty_metric": {"type": "number", "minimum": 0},
            "initial_sigma_min": {"type": "number", "minimum": 0},
            "initial_sigma_max": {"type": "number", "minimum": 0},
            "initial_condition_number": {"type": "number", "minimum": 0},
            "minimum_initial_limit_margin_normalized": {"type": "number"},
            "minimum_initial_limit_margin_absolute_rad": {"type": "number"},
            "controlling_joint_index": {"type": "integer", "minimum": 0, "maximum": 6},
            "candidate_source_pool": {"type": "string", "enum": ["interior", "near_limit", "singular", "stratified"]},
            "source_seed": {"type": "integer"},
            "trajectory_content_hash": {"type": "string", "pattern": SHA256_PATTERN},
            "model_fingerprint": {"type": "string"},
            "config_fingerprint": {"type": "string"},
            "content_hash": {"type": "string", "pattern": SHA256_PATTERN},
        },
